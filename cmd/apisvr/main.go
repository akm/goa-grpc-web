...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[notification] ", log.Ltime)
	}
	...
	{
		messagesSvc = notification.NewMessages(logger)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		messagesEndpoints *messages.Endpoints
	)
	{
		messagesEndpoints = messages.NewEndpoints(messagesSvc)
	}
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, messagesEndpoints, &wg, errc, logger, *dbgF)
		}

		{
			...
			handleGRPCServer(ctx, u, messagesEndpoints, &wg, errc, logger, *dbgF)
		}
